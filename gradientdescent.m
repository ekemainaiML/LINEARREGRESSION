...
% Attribute Information:
 ...
 X_train = [ones(round(0.7*nr),1) dass(1:round(0.7*nr),1:(nc))] ;
 y_train = (dass(1:round(0.7*nr),nc));

 % Gradient descent
 iterations = 1000;
 lrate = 0.001;
 [thetas, cost] = computeGraDesc(iterations, lrate, X_train, y_train, nr, nc);


% Prediction
nr_p = round(0.7*nr) + 1; % Test data index
X_test = [ones((nr - nr_p), 1) dass(nr_p:(nr - 1), 1:(nc))];
y_test = dass(nr_p:(nr - 1), nc);
y_pred = (thetas .') * (X_test .');
